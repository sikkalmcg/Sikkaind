        ...
        if (res.data && res.data.length > 0) {
            ...
            res.data.forEach((v: any) => {
                ...
                gpsMap.set(vNo, {
                    ...
                    ignition: v.ignition,
                    // REGISTRY FIX: Use actual latest location node
        ...
        try {
            ...
            for (const pId of scopePlants) {
                ...
                for (const tripDoc of tripSnap.docs) {
                    ...
                    const arrivalTime = t.arrivalDate instanceof Timestamp ? t.arrivalDate.toDate() : (t.arrivalDate ? new Date(t.arrivalDate) : undefined);
                    const rawStatus = t.tripStatus?.toLowerCase().replace(/[\s_-]+/g, '-') || '';

                    if ((rawStatus === 'arrival-for-delivery' || rawStatus === 'arrived-at-destination') && arrivalTime && arrivalTime >= dayStart && arrivalTime <= dayEnd) {
            ...
            console.error("Arrival fetch failure:", error);
        } finally {
            setLoading(false);
        }
    };
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] w-full h-[90vh] flex flex-col p-0 border-none shadow-3xl overflow-hidden bg-white rounded-3xl">
        <DialogHeader className="p-6 bg-slate-900 text-white shrink-0">
          <DialogTitle className="text-xl font-black uppercase tracking-tight italic">Arrival Vehicles Registry</DialogTitle>
            ...
            <ScrollArea className="h-full">
                ...
                <div className="rounded-3xl border-2 border-slate-200 bg-white shadow-xl overflow-hidden">
                    ...
                    <TableHeader className="sticky top-0 bg-slate-100 z-10 border-b-2">
                        ...
                        <TableHead className="text-[10px] font-black uppercase px-4 text-center text-slate-500">Arrival Date/Time</TableHead>
                        <TableHead className="text-[10px] font-black uppercase px-4 text-slate-500">Ship To Party</TableHead>
                        <TableHead className="text-[10px] font-black uppercase px-4 text-right text-slate-500">Manifest Qty</TableHead>
                        <TableHead className="text-[10px] font-black uppercase px-6 text-right text-slate-500">Action</TableHead>
                        </TableRow>
                    </TableHeader>
                    <TableBody>
                        {loading ? (
                        ...
                        <TableRow>
                            <TableCell colSpan={8} className="h-64 text-center text-slate-400 italic font-medium uppercase tracking-widest opacity-40">No arrivals matching current criteria.</TableCell>
                        </TableRow>
                        ) : (
                        filteredData.map((item, index) => {
                            ...
                            return (
                                ...
                                <TableCell className="px-4 text-center text-[10px] font-bold text-slate-400 whitespace-nowrap">{item.arrivalDate ? format(new Date(item.arrivalDate), 'dd/MM/yy HH:mm') : 'N/A'}</TableCell>
        ...
        <DialogFooter className="p-6 bg-slate-50 border-t flex-row items-center justify-between sm:justify-between shrink-0">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ShieldCheck className="h-4 w-4 text-emerald-600" /> Arrival Telemetry Verified
            </p>
