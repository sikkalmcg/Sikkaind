    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[95vw] w-full h-[90vh] flex flex-col p-0 border-none shadow-3xl overflow-hidden bg-white rounded-3xl">
        <DialogHeader className="p-6 bg-slate-900 text-white shrink-0">
          <DialogTitle className="text-xl font-black uppercase tracking-tight italic">Assigned Vehicles Registry</DialogTitle>
          <DialogDescription className="text-blue-300 font-bold uppercase text-[9px] tracking-widest mt-1">
            Plant: {plantName} | Mission Allocation Monitor
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 flex flex-col overflow-hidden bg-[#f8fafc]">
          <div className="p-6 bg-white border-b shrink-0">
            <div className="relative max-w-md">
              ...
              <Input
                placeholder="Search registry or area..."
                value={searchTerm}
            ...
            <ScrollArea className="h-full">
                ...
                <div className="rounded-3xl border-2 border-slate-200 bg-white shadow-xl overflow-hidden">
                    ...
                    <TableBody>
                        ...
                        <TableRow>
                            <TableCell colSpan={8} className="h-64 text-center text-slate-400 italic font-medium uppercase tracking-widest opacity-40">No active assignments found.</TableCell>
                        </TableRow>
                        ) : (
                        filteredData.map((item) => {
                            ...
                            return (
                                ...
                                <TableCell className="px-4 text-center">
                                    <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 font-black uppercase text-[9px] px-3 shadow-sm">
                                        {item.tripStatus}
                                    </Badge>
                                </TableCell>
                                <TableCell className="px-6 text-right">
                                    <Button asChild variant="ghost" size="sm" className="h-8 rounded-lg font-black text-[10px] uppercase text-blue-600 hover:bg-blue-100">
                                        <a href={`/dashboard/shipment-tracking?search=${item.vehicleNumber}`} target="_blank" rel="noopener noreferrer">Live Monitor</a>
                                    </Button>
                                </TableCell>
                                </TableRow>
                            );
        ...
        <DialogFooter className="p-6 bg-slate-50 border-t flex-row items-center justify-between sm:justify-between shrink-0">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                <ShieldCheck className="h-4 w-4 text-blue-600" /> Assignment Hierarchy Verified
            </p>
