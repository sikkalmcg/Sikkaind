        ...
        <Table className="border-collapse w-full min-w-[1400px]">
          <TableHeader className="sticky top-0 z-50">
            <TableRow className="h-14 hover:bg-transparent border-b-2 border-slate-200 shadow-[0_2px_5px_rgba(0,0,0,0.05)]">
              ...
              <TableHead className="text-[10px] font-black uppercase px-4 text-slate-500 w-36 bg-slate-100">Sales Order No</TableHead>
          ...
          <TableBody>
            {data.length === 0 ? (
              <TableRow><TableCell colSpan={14} className="h-64 text-center text-slate-400 italic font-medium uppercase tracking-[0.3em] opacity-40">No mission nodes detected.</TableCell></TableRow>
            ) : (
              data.map((order) => {
                ...
                return (
                  <React.Fragment key={order.id}>
                    <TableRow className="h-16 border-b border-slate-100 last:border-0 hover:bg-blue-50/20 even:bg-slate-50/30 transition-all group text-[11px] font-medium text-slate-600">
                      ...
                      <TableCell className="px-4 truncate font-black text-slate-900 uppercase text-xs" title={order.unloadingPoint}>{getCity(order.unloadingPoint)}</TableCell>
                      ...
                      <TableCell className="px-4 text-center"><Badge variant="outline" className={cn("text-[9px] font-black uppercase px-2.5 h-6 border shadow-sm", getStatusColor(order.currentStatusId))}>{order.currentStatusId}</Badge></TableCell>
                      <TableCell className="px-8 text-right sticky right-0 bg-white group-hover:bg-blue-50/20 transition-all shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                        <DropdownMenu modal={false}>
                          <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl hover:bg-white text-slate-400 hover:text-blue-900 transition-all"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger>
                          <DropdownMenuPortal>
                    ...
                    {assignedTrips.map((trip: any, tIdx: number) => (
                        <TableRow key={trip.id} className="h-14 bg-slate-50 hover:bg-blue-50/10 border-b border-slate-100 last:border-slate-200 transition-colors group/trip">
                            <TableCell colSpan={2} className="px-6"><div className="flex items-center gap-3 pl-4"><div className="h-6 w-px bg-slate-200" /><div className="p-1.5 bg-blue-100 rounded-lg"><Truck className="h-3 w-3 text-blue-700" /></div><span className="text-[10px] font-black text-sidebar-foreground/40 uppercase tracking-widest">Node {tIdx + 1}</span></div></TableCell>
                            <TableCell colSpan={2} className="px-4 font-black text-slate-900 uppercase text-xs tracking-tighter">{trip.vehicleNumber}<span className="ml-3 font-mono font-bold text-blue-600 text-[10px] opacity-60">ID: {trip.tripId}</span></TableCell>
                            <TableCell className="px-4"><div className="flex items-center gap-2"><div className="h-5 w-5 rounded-full bg-slate-200 flex items-center justify-center"><UserCircle className="h-3 w-3 text-slate-500" /></div><span className="text-[11px] font-bold text-slate-600 uppercase truncate max-w-[150px]">{trip.driverName || 'Pilot N/A'}</span></div></TableCell>
                            <TableCell colSpan={5} className="px-4"><div className="flex items-center gap-4"><Badge variant="outline" className={cn("text-[8px] font-black uppercase px-2 h-5 border-none", trip.entry?.status === 'OUT' ? "bg-emerald-600 text-white" : "bg-blue-600 text-white")}>{trip.entry?.status === 'OUT' ? 'DISPATCHED' : (trip.entry?.status === 'IN' ? 'IN-YARD' : 'ASSIGNED')}</Badge><span className="text-[9px] font-black text-blue-900 uppercase">Load: {(trip.assignedQtyInTrip || 0).toFixed(3)} MT</span></div></TableCell>
                            <TableCell colSpan={2} className="px-4 text-center"><Badge variant="outline" className="text-[9px] font-black bg-white border-slate-200 text-slate-500 uppercase h-5">{trip.tripStatus || 'Awaiting Node'}</Badge></TableCell>
                            <TableCell className="px-8 text-right sticky right-0 bg-slate-50 group-hover/trip:bg-blue-50/20 transition-all border-l border-slate-100 shadow-[-2px_0_5px_rgba(0,0,0,0.05)]"><div className="flex justify-end gap-1 opacity-0 group-hover/trip:opacity-100 transition-opacity"><TooltipProvider><Tooltip><TooltipTrigger asChild><Button size="icon" variant="ghost" className="h-7 w-7 text-blue-600 hover:bg-white" onClick={() => onEditAssignment(order, trip)} disabled={trip.entry?.status === 'OUT'}><Edit2 className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent className="bg-slate-900 text-white text-[9px] font-black uppercase">Edit Assignment</TooltipContent></Tooltip></TooltipProvider><TooltipProvider><Tooltip><TooltipTrigger asChild><Button size="icon" variant="ghost" className="h-7 w-7 text-red-600 hover:bg-white" onClick={() => onCancelAssignment(trip.id, order.id, trip.assignedQtyInTrip)} disabled={trip.entry?.status === 'OUT'}><Trash2 className="h-3.5 w-3.5" /></Button></TooltipTrigger><TooltipContent className="bg-slate-900 text-white text-[9px] font-black uppercase">Revoke Fleet Node</TooltipContent></Tooltip></TooltipProvider></div></TableCell>
                        </TableRow>
                    ))}
                  </React.Fragment>
                )
