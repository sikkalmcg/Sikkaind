...
export default function VehicleAssignModal({ isOpen, onClose, shipment, trip, onAssignmentComplete, carriers }: VehicleAssignModalProps) {
  ...
  /**
   * ACCURACY HANDSHAKE: Directions Service Implementation
   * Migrated from Distance Matrix to Directions API for project compatibility.
   */
  useEffect(() => {
    if (!isLoaded || !isOpen || !shipment.loadingPoint || !shipment.unloadingPoint || currentDistance > 0) return;
    ...
    calculateDistance();
  }, [isLoaded, isOpen, shipment.loadingPoint, shipment.unloadingPoint, setValue, currentDistance]);
  ...
  useEffect(() => {
    ...
    const lookupVehicle = async () => {
        ...
        if (!snap.empty) {
            ...
            setRegistryMatch(null);
            if (!isEditing) setValue('vehicleType', 'Market Vehicle', { shouldValidate: true });
        }
    };
    const timeout = setTimeout(lookupVehicle, 500);
    return () => clearTimeout(timeout);
  }, [vehicleNumber, firestore, setValue, isNewVehicle, isEditing, toast]);
  ...
  }, [vehicleId, isNewVehicle, vehiclesAtGate, setValue]);
