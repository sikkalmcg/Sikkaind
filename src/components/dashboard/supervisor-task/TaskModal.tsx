...
// RULE: MT removed from UOM options for yard supervisors to focus on package counts
const UOM_OPTIONS = ['Bag', 'Box', 'Drum', 'Pallet', 'Carton', 'Bundle', 'Roll', 'PCS', 'Others'] as const;
...
}).refine(data => data.loadUnit <= data.deliveryUnit, {
    message: "Load Unit cannot exceed Delivery Unit.",
    path: ["loadUnit"]
});
...
export default function TaskModal({ isOpen, onClose, task, onSuccess }: { isOpen: boolean; onClose: () => void; task: any; onSuccess: () => void }) {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: {
            loadingItems: isOutward ? [{ slipNo: '', invoiceNo: '', description: '', deliveryUnit: 0, loadUnit: 0, uom: 'Bag', balanceUnit: 0 }] : [],
            unloadQty: !isOutward ? Number(task.billedQty || 0) : 0,
            remarks: ''
        }
    });
    ...
    const totals = useMemo(() => {
        ...
        const diff = Number((Number(task.billedQty || 0) - unloadQtyValue).toFixed(3));
    ...
    const onSubmit = async (values: z.infer<typeof formSchema>) => {
        ...
        try {
            ...
            const payload: any = {
                ...
                assignedQty: Number(task.assignedQty) || 0,
                billedQty: Number(task.billedQty) || 0,
            ...
            if (isOutward) {
                ...
                payload.unloadQty = values.unloadQty || 0;
                payload.shortExcess = totals.diff;
            }

            const sanitizedPayload = sanitize(payload);
            await addDoc(collection(firestore, `plants/${task.plantId}/supervisor_tasks`), sanitizedPayload);
            
            const entryRef = doc(firestore, 'vehicleEntries', task.id);
            const tripId = task.realTripId; 
            const updateObj: any = {
                    ...
                    <div className="flex justify-between items-center pr-12">
                        <div className="flex items-center gap-4">
                            <div className="p-3 bg-white/10 rounded-2xl backdrop-blur-md">
                                {isOutward ? <Truck className="h-7 w-7 text-blue-400" /> : <Calculator className="h-7 w-7 text-orange-400" />}
                            </div>
                            <div>
                    ...
                    <div className="p-8 bg-white border-b shadow-sm shrink-0">
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-6">
                            {[
                                ...
                                { label: 'Pilot Detail', value: task.driverMobile, icon: Phone, mono: true },
                                ...
                                ...(isOutward ? [
                                    ...
                                    { label: 'Assigned Weight', value: `${task.assignedQty || 0} MT`, icon: Calculator, color: 'text-blue-700', bold: true },
                                ] : [
                                    { label: 'Billed Quantity', value: `${task.billedQty || 0} ${task.qtyType || 'MT'}`, icon: Calculator, color: 'text-orange-700', bold: true },
                                ...
                                {isOutward ? (
                                    <div className="space-y-6">
                                        <div className="flex items-center justify-between px-1">
                                            ...
                                            <Button type="button" variant="outline" size="sm" onClick={() => append({ slipNo: '', invoiceNo: '', description: '', deliveryUnit: 0, loadUnit: 0, uom: 'Bag', balanceUnit: 0 })} className="h-9 px-4 gap-2 font-black text-[10px] uppercase border-blue-200 text-blue-700 bg-white hover:bg-blue-50">
                                                    ...
                                                    {fields.map((field, idx) => (
                                                        <TableRow key={field.id} className="h-16 border-b border-slate-100 last:border-0 hover:bg-blue-50/10 group transition-colors">
                                                            ...
                                                            <TableCell className="px-4 py-2">
                                                                <FormField name={`loadingItems.${idx}.loadUnit`} control={control} render={({ field: itm }) => (
                                                                    <FormItem className="space-y-0">
                                                                        <FormControl><Input type="number" step="0.001" className="h-10 border-slate-200 font-black text-center text-blue-900 focus-visible:ring-blue-900" {...itm} /></FormControl>
                                                            ...
                                                            <TableCell className="px-4 py-2">
                                                                <FormField name={`loadingItems.${idx}.uom`} control={control} render={({ field: itm }) => (
                                                                    <Select onValueChange={itm.onChange} value={itm.value}>
                                                                        <FormControl>
                                                                            <SelectTrigger className="h-10 border-slate-200 font-bold text-[11px]">
                                                            ...
                                                            <TableCell className="pr-6">
                                                                <Button type="button" variant="ghost" size="icon" className="h-8 w-8 text-slate-200 group-hover:text-red-600" onClick={() => remove(idx)} disabled={fields.length === 1}>
                                ...
                                {(isOutward ? totals.balance > 0.001 : (totals as any).diff !== 0) && (
                                    <div className="space-y-4 animate-in slide-in-from-bottom-4 duration-500">
                                        <h3 className="text-xs font-black uppercase tracking-[0.2em] text-red-600 flex items-center gap-2 px-1">
                                            <AlertTriangle className="h-4 w-4 text-red-600" /> 2. Mandatory Discrepancy Justification
                                        </h3>
