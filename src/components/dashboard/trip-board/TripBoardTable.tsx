                ...
                <TableCell className="px-8 text-right sticky right-0 bg-white group-hover:bg-blue-50/20 transition-all shadow-[-4px_0_10px_rgba(0,0,0,0.02)]">
                    <div className="flex justify-end items-center gap-2">
                        ...
                        {activeTab === 'open-order' && (
                            <>
                                {(!row.entry || (row.entry.status !== 'IN' && row.entry.status !== 'OUT')) && (
                                    <Button size="sm" onClick={() => onAction('vehicle-in', row)} className="h-8 bg-blue-600 hover:bg-blue-700 text-white font-black text-[9px] uppercase px-4 rounded-lg">Vehicle In</Button>
                                )}
                                {(row.entry?.status === 'IN' || ['yard/loading', 'yard', 'loading', 'loaded', 'loading-complete'].includes(row.normalizedStatus)) && !row.gateOutDateTime && (
                                    <Button size="sm" onClick={() => onAction('vehicle-out', row)} className="h-8 bg-emerald-600 hover:bg-emerald-700 text-white font-black text-[9px] uppercase px-4 rounded-lg">Vehicle Out</Button>
                                )}
                            </>
                        )}
                        {activeTab === 'rejection' && (
