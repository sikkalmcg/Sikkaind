...
function MissionRegistryCard({ 
    ...
    const handleLRViewClick = async () => {
        if (!row.lrNumber || isArrangeByParty || !firestore) return;
        showLoader();
        try {
            ...
            console.error("LR Extraction Error:", e);
        } finally {
            hideLoader();
        }
    };
                    ...
                    <div className="col-span-2 flex flex-col justify-center">
                        ...
                        <div className="flex items-center gap-2">
                            {row.lrNumber && !isArrangeByParty ? (
                                ...
                                isArrangeByParty ? (
                                    <Badge variant="outline" className="bg-slate-50 text-slate-400 border-slate-200 text-[8px] font-black uppercase px-2 h-5">PARTY MANAGED</Badge>
                                ) : (
                                    canEditLRNode && (
                                        <TooltipProvider>
                                            <Tooltip>
                        ...
                        <DropdownMenu modal={false}>
                            ...
                            <DropdownMenuPortal>
                                <DropdownMenuContent align="end" className="w-56 p-2 rounded-2xl border-slate-200 shadow-3xl bg-white z-[100]">
                                    ...
                                    {showLrAndInvoices && canEditLRNode && !isReadOnly && !isArrangeByParty && (
                                    ...
                                    {(activeTab === 'loading' || activeTab === 'open-order') && !isReadOnly && (
                                        <DropdownMenuItem onClick={() => onAction('change-to-contract', row)} className="gap-3 font-black py-2.5 text-blue-700 rounded-xl cursor-pointer hover:bg-blue-50">
                                            <ArrowRightLeft className="h-4 w-4" /> Change to Contract
                                        </DropdownMenuItem>
                                    )}
