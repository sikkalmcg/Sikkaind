...
import { Save, ChevronLeft, ChevronRight, Search, Printer, Edit3, XCircle } from 'lucide-react';
...
export default function VAPage() {
  ...
  const handleSave = async () => {
    if (activeTCode === 'VA04') {
      ...
      if (!o) return alert('Sale Order registry node not found.');
      ...
      alert('Sale Order successfully short closed in node.');
      setFormData({});
      return;
    }

    if (activeTCode === 'VA01') {
      const exists = orders?.find(o => o.saleOrder === formData.saleOrder);
      if (exists) return alert(`Not Allow duplicate entry Sale order ${formData.saleOrder} is already exit.`);
    }
    ...
    <div className="flex-1 flex flex-col overflow-y-auto p-10 bg-[#f2f2f2] font-mono">
      <div className="bg-white border-b border-slate-300 px-8 py-3 mb-10 shadow-sm flex items-center justify-between">
        <h2 className="text-[16px] font-bold text-slate-800 uppercase italic">
          {activeTCode === 'VA04' ? 'VA04 - Short Close Management' : `${activeTCode} - Sale Order Registry Hub`}
        </h2>
      ...
      {activeTCode === 'VA04' ? (
        <div className="bg-white p-12 border border-slate-300 shadow-sm max-w-4xl mx-auto w-full animate-fade-in">
           <h3 className="text-red-600 font-black uppercase italic mb-8 border-b pb-4 tracking-widest">Short Close Workflow Control</h3>
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 border border-slate-300 shadow-sm flex items-center gap-6 animate-fade-in">
            ...
            <input className="h-9 w-full border border-slate-400 px-4 text-xs font-black uppercase outline-none focus:bg-yellow-50" value={searchId} onChange={e => setSearchId(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { const o = orders?.find(ord => ord.saleOrder === searchId.toUpperCase()); if (o) setFormData(o); } }} placeholder="ENTER SALE ORDER NO..." />
          </div>
                ...
                <tbody className="font-bold uppercase">
                  {paginated.map(o => (
                    <tr key={o.id} onClick={() => setFormData(o)} className="border-b border-slate-100 hover:bg-blue-50 cursor-pointer">
                      ...
                      <td className="p-4 border-r"><Badge variant="outline" className="rounded-none text-[8px] font-black">{getOrderStatus(o)}</Badge></td>
                      ...
                      <td className="p-4 border-r">{parseFloat(o.weight).toFixed(3)} {o.weightUom}</td>
        ...
        <div className="animate-slide-up space-y-12 bg-white p-12 border border-slate-300 shadow-inner">
           <div className="grid grid-cols-2 gap-y-6 gap-x-12">
             ...
             <div className="flex items-center gap-8"><label className="text-[12px] font-bold text-slate-600 w-40 text-right uppercase">Total Weight:</label><input type="number" step="0.001" value={formData.weight || ''} onChange={e => setFormData({...formData, weight: e.target.value})} disabled={isReadOnly && activeTCode !== 'VA02'} className="h-8 w-80 border border-slate-400 px-2 text-[12px] font-black" /></div>
           ...
           {activeTCode === 'VA03' && allTrips?.filter(t => t.saleOrderId === formData.id).length > 0 && (
             <div className="mt-12 space-y-6">
                <h4 className="text-[11px] font-black uppercase text-[#0056d2] italic border-b border-blue-100 pb-2 tracking-widest">Execution Registry: Linked Trip Nodes</h4>
                <div className="overflow-x-auto border border-slate-200 shadow-inner">
                   <table className="w-full text-left text-[10px] min-w-[1800px]">
                      <thead className="bg-[#f8fafc] border-b border-slate-200 font-black uppercase text-slate-500">
                         <tr><th className="p-3 border-r">Plant</th><th className="p-3 border-r">Trip ID</th><th className="p-3 border-r">CN Number</th><th className="p-3 border-r">Fleet</th><th className="p-3 border-r">Vehicle</th><th className="p-3 border-r">Weight</th><th className="p-3 border-r">Vendor</th><th className="p-3">Status</th></tr>
                      </thead>
                      <tbody>
                        {allTrips.filter(t => t.saleOrderId === formData.id).map(t => (
                          <tr key={t.id} className="border-b border-slate-50 font-bold uppercase">
                            <td className="p-3 border-r">{t.plantCode}</td>
                            ...
                            <td className="p-3 border-r font-black">{t.vehicleNumber}</td>
                            <td className="p-3 border-r text-emerald-600">{parseFloat(t.assignWeight).toFixed(3)} MT</td>
