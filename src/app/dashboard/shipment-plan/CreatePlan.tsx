...
<<<<<<< HEAD
import { cn, normalizePlantId, formatSequenceId, generateRandomTripId, incrementSerial, resolvePlantCarrier, matchesPlantReference } from '@/lib/utils';
=======
...
<<<<<<< HEAD
  ...
  const selectedPlant = useMemo(() => {
    return authorizedPlants.find(plant => matchesPlantReference(plant.id, originPlantId) || matchesPlantReference(plant.name, originPlantId));
  }, [authorizedPlants, originPlantId]);

  const plantCarrier = useMemo(() => {
    if (!originPlantId || !carriers) return null;
    return resolvePlantCarrier(
      carriers,
      originPlantId,
      selectedCarrierId,
      [selectedPlant?.name]
    );
  }, [originPlantId, carriers, selectedCarrierId, selectedPlant?.name]);

  useEffect(() => {
    if (plantCarrier && plantCarrier.id !== selectedCarrierId) {
      setValue('carrierId', plantCarrier.id, { shouldValidate: true });
    } else if (!plantCarrier && selectedCarrierId) {
      setValue('carrierId', '', { shouldValidate: true });
    }
  }, [plantCarrier, selectedCarrierId, setValue]);
=======
