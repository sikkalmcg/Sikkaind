...
export default function CreatePlan({ onShipmentCreated }: CreatePlanProps) {
  ...
  const handlePost = async (values: FormValues) => {
    ...
    // REGISTRY DUPLICATE CHECK: LR Number node uniqueness
    if (values.lrNumber?.trim()) {
        ...
        if (!snap.empty) {
            toast({ variant: 'destructive', title: "Registry Conflict", description: `Duplicate LR Number - This LR Number ${values.lrNumber} already exists for the selected Carrier.` });
            return;
        }
    }
            ...
            for (let i = 0; i < jsonData.length; i++) {
                ...
                try {
                    ...
                    const docData: any = {
                        ...
                        shipmentId,
                        currentStatusId: lrNumber ? 'Assigned' : 'pending',
