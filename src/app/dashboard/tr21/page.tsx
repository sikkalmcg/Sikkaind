...
export default function TR21Page() {
  ...
  const generateCnPreview = (trip: any) => {
    ...
    const carrier = companies?.[0]; // Default carrier
                  ...
                  <td className="p-3">
                    {activeTab === 'Open Orders' ? (
                      ...
                      <div className="flex gap-2">
                        {['Arrived', 'POD Verify', 'Closed'].includes(activeTab) && <Button onClick={() => generateCnPreview(item)} variant="outline" className="h-7 px-3 text-[9px] font-black rounded-none uppercase border-slate-300"><Printer className="h-3.5 w-3.5 mr-1" /> Document</Button>}
                        <Button 
                  ...
                  <div className="border-[3px] border-black h-full flex flex-col p-8 relative">
                    <div className="flex justify-between items-start border-b-[3px] border-black pb-6 mb-8">
                       <div className="space-y-2">
                          ...
                          <div className="text-[9px] font-bold text-slate-400 uppercase leading-tight">GSTIN: {cnData.carrier?.gstin || '09SIKKA7845L1Z0'}<br/>PAN: {cnData.carrier?.pan || '-'}</div>
                       </div>
