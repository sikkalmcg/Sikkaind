...
import { Printer, Save, ChevronLeft, ChevronRight, X, Download, AlertTriangle, CheckCircle } from 'lucide-react';
...
export default function TR21Page() {
  ...
  const filteredData = React.useMemo(() => {
    ...
    if (activeTab === 'Open Orders') {
      // Fix: Filter out Short closed orders (e.g. 2000129453)
  ...
  // CN Number Logic
  const getNextCnNumber = (vendorName: string) => {
    const carrierTrips = trips?.filter(t => t.vendorName === vendorName && t.cnNumber) || [];
    if (carrierTrips.length === 0) return 'CN-0001';
    const lastCn = carrierTrips.sort((a, b) => b.cnNumber.localeCompare(a.cnNumber))[0].cnNumber;
    const lastNum = parseInt(lastCn.split('-')[1]) || 0;
  ...
  const validateDateTime = (trip: any, newStatus: string, newDate: string) => {
    ...
    if (isBefore(nd, assignDate)) return 'Status date cannot be before Assignment date.';
    
    if (newStatus === 'IN-TRANSIT' && trip.outDate && isBefore(nd, parseISO(trip.outDate))) return 'In-Transit date must be after Vehicle Out date.';
    if (newStatus === 'ARRIVED' && trip.inTransitDate && isBefore(nd, parseISO(trip.inTransitDate))) return 'Arrived date must be after In-Transit date.';
    
    return null;
  };

  const handleAssign = (data: any) => {
    // CN Duplicate Validation
    if (data.cnNumber && trips?.some(t => t.cnNumber === data.cnNumber && t.vendorName === data.vendorName)) {
      alert(`Duplicate CN Number: ${data.cnNumber} already exists for this carrier.`);
      return;
    }
  ...
  const handleStatusUpdate = () => {
    const error = validateDateTime(selectedItem, nextStatus, statusDate);
    if (error) {
      alert(error);
      return;
    }
    ...
    if (nextStatus === 'IN-TRANSIT') updates.outDate = statusDate;
    if (nextStatus === 'ARRIVED') updates.arrivedDate = statusDate;
    if (nextStatus === 'CLOSED') updates.closedDate = statusDate;

    setDocumentNonBlocking(doc(db, 'users', SHARED_HUB_ID, 'trips', selectedItem.id), updates, { merge: true });
    setShowStatusUpdate(false);
  };

  const generateCn = (trip: any) => {
    const consignor = customers?.find(c => c.customerName === trip.consignor || (c.customerName + ' - ' + c.city) === trip.consignor);
    const shipTo = customers?.find(c => c.customerName === trip.shipToParty || (c.customerName + ' - ' + c.city) === trip.shipToParty);
    const consignee = customers?.find(c => c.customerName === trip.consignee || (c.customerName + ' - ' + c.city) === trip.consignee);
  ...
  const downloadPdf = () => {
    const filename = `${cnData.cnNumber || 'CN_DOC'}.pdf`;
    // Web implementation of auto-download trigger
    window.print();
  };
    ...
    <div className="flex-1 flex flex-col bg-[#f2f2f2] font-mono overflow-hidden">
                  ...
                  <td className="p-3">
                    {activeTab === 'Open Orders' ? (
                      ...
                      <div className="flex gap-2">
                        {['Arrived', 'POD Verify', 'Closed'].includes(activeTab) && <Button onClick={() => generateCn(item)} variant="outline" className="h-7 px-3 text-[9px] font-black rounded-none uppercase"><Printer className="h-3 w-3 mr-1" /> CN</Button>}
                        <Button 
                          onClick={() => { setSelectedItem(item); setNextStatus(item.status); setShowStatusUpdate(true); }}
                          variant="ghost" className="h-7 text-[9px] font-black border border-slate-200 uppercase rounded-none"
                        >Update</Button>
                      </div>
                    )}
                  </td>
        ...
        <DialogContent className="max-w-2xl rounded-none border-[4px] border-[#0056d2] font-mono">
           ...
           <div className="p-6 space-y-6">
              <div className="grid grid-cols-2 gap-6">
                 ...
                 <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">CN Number (Auto-fill)</label><input id="cn" defaultValue={getNextCnNumber('OWN FLEET')} className="h-9 w-full border border-slate-400 px-3 text-xs font-black uppercase" /></div>
              </div>
              <Button onClick={() => {
                const data = {
      ...
      <Dialog open={showStatusUpdate} onOpenChange={setShowStatusUpdate}>
        <DialogContent className="max-w-md rounded-none border-[4px] border-slate-800 font-mono">
           <DialogHeader><DialogTitle className="text-sm font-black uppercase italic">Life-cycle Status Synchronization</DialogTitle></DialogHeader>
           <div className="p-6 space-y-6">
              <div className="space-y-4">
                 <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Next Status</label><select value={nextStatus} onChange={e => setNextStatus(e.target.value)} className="h-9 w-full border border-slate-400 px-2 text-xs font-black"><option value="LOADING">LOADING</option><option value="IN-TRANSIT">IN-TRANSIT (OUT)</option><option value="ARRIVED">ARRIVED</option><option value="CLOSED">CLOSED (UNLOAD)</option></select></div>
                 <div className="space-y-1"><label className="text-[10px] font-black text-slate-400 uppercase">Status Date Time *</label><input type="datetime-local" value={statusDate} onChange={e => setStatusDate(e.target.value)} className="h-9 w-full border border-slate-400 px-3 text-xs font-black" /></div>
              </div>
              <Button onClick={handleStatusUpdate} className="w-full h-10 bg-slate-900 text-white rounded-none font-black uppercase text-[11px]">Commit Sequence</Button>
           </div>
        </DialogContent>
      </Dialog>
        ...
        <DialogContent className="max-w-[1000px] max-h-[90vh] overflow-y-auto rounded-none p-0 border-none font-mono">
           <div className="bg-slate-800 p-4 sticky top-0 z-50 flex justify-between items-center print:hidden">
              ...
              <div className="flex gap-4">
                <Button onClick={downloadPdf} className="h-8 bg-emerald-600 rounded-none text-[10px] font-black uppercase px-6"><Download className="h-3.5 w-3.5 mr-2" /> Download/Print A4</Button>
           ...
           <div id="printable-area" className="bg-white p-0 space-y-0">
              {(cnData.copies || []).map((copy: string) => (
                <div key={copy} className="cn-print-page border-[1.5px] border-black p-8 flex flex-col relative min-h-[1100px] print:m-0 print:border-none">
                  <div className="border-2 border-black h-full flex flex-col p-6">
