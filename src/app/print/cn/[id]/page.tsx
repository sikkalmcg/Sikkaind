...
export default function CNPrintPage() {
  ...
  // 3 से ज़्यादा मटेरियल होने पर इनवॉइस और ई-वेबिल को 2-2 के जोड़े (Pairs) में प्रोसेस करने का लॉजिक
  const processedInvoices = (() => {
    const rawInvoices = (trip.invoices || []).filter((inv: any) => inv.invNo);
    
    if (rawInvoices.length <= 3) {
      return rawInvoices;
    }

    const pairedInvoices: any[] = [];
    
    for (let i = 0; i < rawInvoices.length; i += 2) {
      const first = rawInvoices[i];
      const second = rawInvoices[i + 1];

      if (second) {
        pairedInvoices.push({
          invNo: `${first.invNo}, ${second.invNo}`,
          ewaybillNo: `${first.ewaybillNo || '-'},\n${second.ewaybillNo || '-'}`,
          desc: first.desc === second.desc ? first.desc : `${first.desc} / ${second.desc}`,
          pkg: (parseInt(first.pkg) || 0) + (parseInt(second.pkg) || 0),
          uom: first.uom || second.uom || 'PKG',
          isPaired: true,
          originalWeight: (parseFloat(first.weight) || 0) + (parseFloat(second.weight) || 0)
        });
      } else {
        pairedInvoices.push({
          ...first,
          ...
          <div key={index} className="cn-page bg-white text-black font-normal p-[15mm] min-h-[297mm] flex flex-col relative overflow-hidden print:overflow-visible print:border-none">
            ...
            <div className="grid grid-cols-3 gap-0 mb-4 border border-black text-black shrink-0">
               <div className="border-r border-black p-3 space-y-3 min-h-[140px]">
                  ...
                  <div className="text-[10px] uppercase font-normal space-y-1 text-black">
                     ...
                     {consignor?.mobile && <p className="text-[10px] text-black font-normal">MOB: {consignor.mobile}</p>}
                  </div>
               </div>
               <div className="border-r border-black p-3 space-y-3 min-h-[140px]">
                  ...
                  <div className="text-[10px] uppercase font-normal space-y-1 text-black">
                     ...
                     {consignee?.mobile && <p className="text-[10px] text-black font-normal">MOB: {consignee.mobile}</p>}
                  </div>
               </div>
               <div className="p-3 space-y-3 min-h-[140px] bg-white text-black">
                  ...
                  <div className="text-[10px] uppercase font-normal space-y-1 text-black">
                     ...
                     <p className="text-[10px] text-black font-normal">MOB: {trip.shipToPartyData?.mobile || trip.shipToPartyData?.customerMobile || shipToParty?.mobile || '-'}</p>
                  </div>
               </div>
            </div>
            ...
            <div className="mb-4 text-black shrink-0">
               <table className="w-full border-collapse border border-black text-[11px]">
                  ...
                  <tbody>
                     {processedInvoices.map((inv: any, i: number) => (
                        <tr key={i} className="border-b border-black last:border-b-0 uppercase font-normal text-black text-[11px]">
                           <td className="p-2 border-r border-black whitespace-pre-line">{inv.invNo}</td>
                           <td className="p-2 border-r border-black whitespace-pre-line">{inv.ewaybillNo}</td>
